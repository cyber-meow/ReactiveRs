...
pub fn new_pure_signal<S>() -> NewPureSignal<S> {
    NewPureSignal {
        signal: PhantomData,
    }
}
...
fn create_new_signal() {
    let s: NewPureSignal<PureSignal> = new_pure_signal();
    let g = s.pause();
}

pub struct NewPureSignal<S> {
    signal: PhantomData<S>,
}

impl<S> Process for NewPureSignal<S> where S: Signal {
    type Value = S;
}

impl ProcessSt for NewPureSignal<PureSignal> {
