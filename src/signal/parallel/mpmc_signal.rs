    ...
    where B: Clone + Send + Sync + 'static, F: Send + Sync + 'static
{
    ...
    fn emit<A>(&mut self, runtime: &mut ParallelRuntime, emitted: A) where F: FnMut(A, &mut B) {
...
impl<B, F> Signal for MpmcSignalImpl<B, F>
    where B: Clone + Send + Sync + 'static, F: Send + Sync + 'static
{
...
impl<B, F> MpmcSignal for MpmcSignalImpl<B, F>
    where B: Clone + Send + Sync + 'static, F: Send + Sync + 'static
{
    type Stored = B;
    type Gather = F;
...
impl<A, B, F> ConstraintOnValue for Emit<MpmcSignalImpl<B, F>, A> {
    type T = ();
}

impl<A, B, F> ProcessPl for Emit<MpmcSignalImpl<B, F>, A>
...
impl<A, B, F> ProcessMutPl for Emit<MpmcSignalImpl<B, F>, A>
...
impl<B, F> ConstraintOnValue for Await<MpmcSignalImpl<B, F>> where B: Send + Sync {
    type T = B;
}

impl<B, F> ProcessPl for Await<MpmcSignalImpl<B, F>>
...
impl <B, F> ProcessMutPl for Await<MpmcSignalImpl<B, F>>
    where B: Clone + Send + Sync + 'static, F: 'static + Send + Sync
{
    fn call_mut<C>(self, runtime: &mut ParallelRuntime, next: C)
        where Self: Sized, C: ContinuationPl<(Self, Self::Value)>
